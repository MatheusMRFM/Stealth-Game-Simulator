...
void Inimigo::gera_posicao_inicial (float tam_cenario, Agente *fugitivo, Parede **parede, int num_parede) {
	...
	float dx, dy, dist;
	while (!fim) {
		this->path.start.x = rand() % (int)tam_cenario/2.0;
		this->path.start.y = rand() % (int)tam_cenario/2.0;
		...
		if (this->path.mesh_start != -1 && dist > DIST_AGENTE_INICIAL)
			fim = true;
	}
...
void Inimigo::define_novo_destino (Cenario *cenario) {
	...
	if (this->path.terminou_caminho) {
		...
		if (this->espera == 0) {
			...
			Ponto_Mapa *p = this->gera_posicao(cenario->tamanho, this->id + this->num_percursos);
			this->num_percursos++;
